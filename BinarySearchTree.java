...
public class BinarySearchTree {
	...
	/*//Method to find the Floor of the given node in the given BST
	public BSTNode getFloorOfNode(BSTNode root, BSTNode node){
		if(root==null)
			return root;
		BSTNode floor = getFloorOfNodeUtil(root, node);
		return floor;
	}
	
	//Utility Method for getFloorOfNode
	public BSTNode getFloorOfNode(BSTNode root, BSTNode node){
		
	}*/
	
	//Method to return the Ceiling of the given node in the given BST
	public int getCeil(BSTNode root, int node){
		if(root==null)
			return -1;
		if(root.val==node)
			return node;
		if(root.val<node){
			return (getCeil(root.right, node));
		}
		int res = getCeil(root.left,node);
		if(res>=node)
			return res;
		else
			return root.val;
	...
	public static void main(String args[]){
		...
		BSTNode t4 = new BSTNode(5);
		BSTNode t5 = new BSTNode(50);
		BSTNode t6 = new BSTNode(62);
		...
		//obj.display(t4);
		System.out.println(obj.getCeil(t4,3));
		//obj.display(t4);
